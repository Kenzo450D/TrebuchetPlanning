...
def getPositionsOfSpheres(turtlebot, radius):
    q = turtlebot.getConfig()
    spherePos = []
    angle = q[2]
    for index in range(0,4):
        angle = q[2] + index*(3.14159/2)
        sx = q[0] + radius * math.cos(angle)
        sy = q[1] + radius * math.sin(angle)
        spherePos.append([sx,sy])
    return spherePos

def setSpheres(curRobot, sphereList, radius) :
    spherePos = getPositionsOfSpheres(curRobot, radius)
    for idx,item in enumerate(spherePos):
        qTmp = [item[0],item[1],0.2,0,0,0]
        sphereList[idx].setConfig(qTmp)
...
if __name__ == "__main__":
    ...
    turtleBot = turtlebot(world.robot(2), "tbot", vis)
    turtleBot.setAltitude(0.02)
    sphero1 = sphero6DoF(world.robot(3), "sphero1", vis)
    sphero2 = sphero6DoF(world.robot(4), "sphero2", vis)
    sphero3 = sphero6DoF(world.robot(5), "sphero3", vis)
    sphero4 = sphero6DoF(world.robot(6), "sphero2", vis)
    ...
    # Radius presets are the radius of the circular formation
    # kobukiL has an approximate radius of 1
    # kobukiS has an approximate radius of 0.4
    radiusPresets = [1,0.4]
    
    allRobotList = []
    allRobotList.append(kobukiL)
    allRobotList.append(kobukiS)
    allRobotList.append(turtleBot)
    allRobotList.append(sphero1)
    allRobotList.append(sphero2)
    allRobotList.append(sphero3)
    allRobotList.append(sphero4)
    
    sphereList = []
    sphereList.append(sphero1)
    sphereList.append(sphero2)
    sphereList.append(sphero3)
    sphereList.append(sphero4)
    ...
    noCollisionConfig.append([ -5.5, -5.5, 0.0, 0.0, 0.0, 0.0]) #sphere1
    noCollisionConfig.append([ -4.5, -4.5, 0.0, 0.0, 0.0, 0.0]) #sphere2
    noCollisionConfig.append([ -5.0, -4.5, 0.0, 0.0, 0.0, 0.0]) #sphere3
    noCollisionConfig.append([ -4.5, -5.0, 0.0, 0.0, 0.0, 0.0]) #sphere4
    ...
    setSpheres(curRobot, sphereList, radiusPresets[robotConfigs[0]])
    ...
    count = 0
    currentRadius = radiusPresets[robotConfigs[0]]
    deltaRadius = (max(radiusPresets) - min(radiusPresets))/1000
    while vis.shown() and (time.time() - startTime < simTime):
        ...
        if changeConfigurationOfRobot == True :
            ...
            diffRadius = currentRadius - radiusPresets[rId]
            RADIUSEPS
            if (abs(diffRadius) > RADIUSEPS) and  (currentRadius < radiusPresets[rId]):
                currentRadius = currentRadius + deltaRadius
            elif (abs(diffRadius) > RADIUSEPS) and (currentRadius > radiusPresets[rId]):
                currentRadius = currentRadius - deltaRadius
            elif (abs(diffRadius) < RADIUSEPS) :
                currentRadius = radiusPresets[rId]
                changeConfigurationOfRobot = False
            setSpheres(curRobot, sphereList, currentRadius)
            print "Current Radius: ", currentRadius
            print "Required Radius: ", radiusPresets[rId]
