#!/usr/bin/python
##Author: Saurav Agarwal Sayantan Datta
##E-mail: sagarw10@uncc.edu sayantan.knz@gmail.com
...
if __name__ == "__main__":
    ...
    ttGoalPos = [-3,1,0]
    tbot1 = turtlebot(world.robot(0), "turtleBot1", vis)
    tbot1.setAltitude(0.02)
    tbot2 = turtlebot(world.robot(1), "turtleBot2", vis)
    tbot2.setAltitude(0.02)
    robot = kobuki(world.robot(2), "KobukiBot", vis)
    robot.setAltitude(0.15)
    robot.setConfig(ttStartPos)
    # -- set the turtlebots outside the map
    tbot1.setConfig([-5,-5,0])
    ...
    trajPlan = RRT(robot, world, collisionChecker,  ttStartPos, ttGoalPos, 2)
